...
export class InstanceDataQueriesService {
    ...
    public getInstanceDataForLocation(collectionId: string, instanceId: string, locId: string, datetime?: string, crs?: string, f?: string, observe: any = 'body', reportProgress: boolean = false, options?: {httpHeaderAccept?: 'application/prs.coverage+json' | 'application/geo+json' | 'application/x-netcdf' | 'text/xml' | 'application/json' | 'text/html'}): Observable<any> {
        ...
        if (locId === null || locId === undefined) {
            throw new Error('Required parameter locId was null or undefined when calling getInstanceDataForLocation.');
        }
        ...
        return this.httpClient.get<CoverageJSON>(`${this.configuration.basePath}/collections/${encodeURIComponent(String(collectionId))}/instances/${encodeURIComponent(String(instanceId))}/locations/${encodeURIComponent(String(locId))}`,
            {
                params: queryParameters,
                responseType: <any>responseType,
                withCredentials: this.configuration.withCredentials,
